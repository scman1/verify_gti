...
manualedit_dir = Path(Path().absolute().parent, base_dir, "full_set")
...
finished_dir = Path(Path().absolute().parent, base_dir,"final_pass1")
...
#verify_instance_labels_match(finished_dir)
# 7. rewrite all pngs, eliminate alpha channel if it was added on manual edit
print("7 rewrite pngs, remove alpha channels if present")
rezise_png_images(finished_dir)
# 8. check pixel dimensions
print("8 final image dimension check")
list_image_size(finished_dir)
finished2_dir = Path(Path().absolute().parent, base_dir,"final_pass2")
print("8.1 label colours")
