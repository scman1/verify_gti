...
def standard_based_dpi(img_height, img_width):
    ...
    for i in standard_dpi:
        ...
        if calc_width > max_width*.95 and calc_width < max_width*1.05:
            return i
    if predicted_dpi == 0:
        for i in standard_dpi:
            calc_height = img_height/i
            if calc_height > max_height*.95 and calc_height < max_height*1.05:
                return i
...
def pixel_sizes(directory, width_to, height_to ):
    ...
    for filepath in sorted(directory.glob('*')):
        ...
        if height!=height_to or width != width_to:
            ...
            if width > width_to or height > height_to:
                print("need to crop width")
                crop_images(filepath, width_to, height_to)
            elif height < height_to:
                print("need to extend height")
                extend_images(filepath, width_to, height_to)
...
def extend_images(filepath, width_to, height_to):
    print("extending to",width_to, height_to)
    #read the image
    img = cv2.imread(str(filepath), cv2.IMREAD_COLOR)
    height,width,_ = img.shape
    pix_add_x = width_to - width
    pix_add_y = height_to - height
    resized_img= cv2.copyMakeBorder(img,0,pix_add_y,0,pix_add_x,cv2.BORDER_CONSTANT,value=0)
    if "JPG" in filepath.name:
        cv2.imwrite(str(filepath),resized_img,params_jpg)
    elif "png" in filepath.name:
        cv2.imwrite(str(filepath),resized_img,params_png)
...
# rotate files and save as jpg
def rotate_files(source_dir, dest_dir):
    dest_dir.mkdir(parents=True, exist_ok=True)
    for filepath in sorted(source_dir.glob('*.tiff')):
        img = cv2.imread(str(filepath))
        img_rotation =numpy.rot90(img)
        rotated_file = Path(dest_dir,filepath.name.replace("tiff","JPG"))
        cv2.imwrite(str(rotated_file),img_rotation,params_jpg)
