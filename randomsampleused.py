...
def copysample(samples_dir, dest_dir, file_list):
...
base_path = "herbariumsheets/mix_unlabelled"
used_dir = Path(Path().absolute().parent,base_path)

random_dir = Path(Path().absolute().parent,base_path, "random")

list_files = []
for filename in used_dir.glob("*.JPG"):
   list_files.append(filename.name)
print("Image total: ", len(list_files))

###get a random sample from the non used list to create a new test set
randomlist = randomsample(list_files, 1000)
##
copysample(used_dir, random_dir, randomlist)
